export const getSeries = (name) => {
  return series.find(item => item.id === `series-${name.toLowerCase()}`);
}

export const getBlog = (id) => {
  return blogs.find(item => item.id === Number(id));
}

export const categories = [
    {
      id: 1,
      name: "Romance",
      description: "Love-filled stories that touch the heart.",
      imageUrl: "https://images.pexels.com/photos/18396/pexels-photo.jpg",
      href: "/category/Romance",
    },
    {
      id: 2,
      name: "Thriller",
      description: "Gripping tales full of suspense and mystery.",
      imageUrl: "https://images.pexels.com/photos/11104885/pexels-photo-11104885.jpeg",
      href: "/category/Thriller",
    },
    {
      id: 3,
      name: "Fantasy",
      description: "Enter magical worlds and epic adventures.",
      imageUrl: "https://images.pexels.com/photos/3115525/pexels-photo-3115525.jpeg",
      href: "/category/Fantasy",
    },
    {
      id: 4,
      name: "Drama",
      description: "Intense, emotional stories full of depth.",
      imageUrl: "https://images.pexels.com/photos/12932543/pexels-photo-12932543.jpeg",
      href: "/category/Drama",
    },
    {
      id: 5,
      name: "Comedy",
      description: "Lighthearted stories to make you smile.",
      imageUrl: "https://images.pexels.com/photos/1890149/pexels-photo-1890149.jpeg",
      href: "/category/Comedy",
    },
    {
      id: 6,
      name: "Adventure",
      description: "Exciting journeys and daring challenges.",
      imageUrl: "https://images.pexels.com/photos/6260556/pexels-photo-6260556.jpeg",
      href: "/category/Adventure",
    },
];


const series = [
  {
    id: "series-desire",
    title: "Desire",
    authors: ["Jane Doe"],
    noOfChapters: 3,
    imageUrl: "https://images.pexels.com/photos/3050232/pexels-photo-3050232.jpeg",
    description: "A passionate tale of longing, hidden promises, and flames of love set under Parisian skies.",
    href: "/series/Desire",
    categories: ["Romance", "Drama"],
    upvotes: 344, // sum of all Desire blogs
    downvotes: 23,
    rating: 4.6,  // avg rating across chapters
  },
  {
    id: "series-eternal",
    title: "Eternal",
    authors: ["Daniel Carter"],
    noOfChapters: 4,
    imageUrl: "https://images.pexels.com/photos/381739/pexels-photo-381739.jpeg",
    description: "A legendary journey of friendship, fire, frost, and eternal destiny.",
    href: "/series/Eternal",
    categories: ["Adventure", "Fantasy", "Drama"],
    upvotes: 500,
    downvotes: 32,
    rating: 4.6,
  },
  {
    id: "series-dark",
    title: "Dark",
    authors: ["Alice Brown"],
    noOfChapters: 1,
    imageUrl: "https://images.pexels.com/photos/8370720/pexels-photo-8370720.jpeg",
    description: "A single but powerful chapter of forgotten desires under the moonlight.",
    href: "/series/Dark",
    categories: ["Drama", "Romance"],
    upvotes: 142,
    downvotes: 12,
    rating: 4.8,
  },
  {
    id: "series-confessions",
    title: "Confessions",
    authors: ["James Parker"],
    noOfChapters: 1,
    imageUrl: "https://images.pexels.com/photos/6706946/pexels-photo-6706946.jpeg",
    description: "A heartfelt confession told across breathtaking sunsets.",
    href: "/series/Confessions",
    categories: ["Adventure", "Drama"],
    upvotes: 110,
    downvotes: 6,
    rating: 4.5,
  }
];


const blogs = [
  {
    id: 1,
    title: "Velvet Whispers: A Night in Paris",
    author: "Jane Doe",
    readTime: "6 min read",
    description: "A tender, slow-burn encounter set under the Parisian moon—our editor's top pick for this week.",
    imageUrl: "https://images.pexels.com/photos/3050232/pexels-photo-3050232.jpeg",
    href: "/story/1",
    categories: ["Romance", "Drama"],
    series: {
      ...getSeries("desire"),
      chapter: 1,
      chapterName: "A Night in Paris",
    },
    upvotes: 124,
    downvotes: 8,
    rating: 4.7,
  },
  {
    id: 2,
    title: "Crimson Petals",
    author: "John Smith",
    readTime: "4 min read",
    description: "An intimate story of longing and passion unfolding in a secret garden.",
    imageUrl: "https://images.pexels.com/photos/285938/pexels-photo-285938.jpeg",
    href: "/story/2",
    categories: ["Romance", "Fantasy"],
    upvotes: 98,
    downvotes: 5,
    rating: 4.3,
  },
  {
    id: 3,
    title: "Moonlit Desire",
    author: "Alice Brown",
    readTime: "5 min read",
    description: "A slow dance beneath the moonlight that awakens forgotten desires.",
    imageUrl: "https://images.pexels.com/photos/8370720/pexels-photo-8370720.jpeg",
    href: "/story/3",
    categories: ["Drama", "Romance"],
    series: {
      ...getSeries("dark"),
      chapter: 2,
      chapterName: "Moonlit Desire",
    },
    upvotes: 142,
    downvotes: 12,
    rating: 4.8,
  },
  {
    id: 4,
    title: "The Secret Garden of Dreams",
    author: "Amelia Clark",
    readTime: "6 min read",
    imageUrl: "https://images.pexels.com/photos/1700765/pexels-photo-1700765.jpeg",
    description: "A soothing escape into nature’s hidden corners...",
    href: "/story/4",
    categories: ["Fantasy", "Adventure"],
    upvotes: 87,
    downvotes: 3,
    rating: 4.1,
  },
  {
    id: 5,
    title: "Chasing Sunsets Across the World",
    author: "James Parker",
    readTime: "5 min read",
    imageUrl: "https://images.pexels.com/photos/6706946/pexels-photo-6706946.jpeg",
    description: "A journey through the most breathtaking sunsets.",
    href: "/story/5",
    categories: ["Adventure", "Drama"],
    series: {
      ...getSeries("confessions"),
      chapter: 3,
      chapterName: "Chasing Sunsets",
    },
    upvotes: 110,
    downvotes: 6,
    rating: 4.5,
  },
  {
    id: 6,
    title: "The Forgotten Letters",
    author: "Sophia Bennett",
    readTime: "7 min read",
    imageUrl: "https://images.pexels.com/photos/5988265/pexels-photo-5988265.jpeg",
    description: "An old box of letters unravels a hidden past...",
    href: "/story/6",
    categories: ["Drama", "Thriller"],
    upvotes: 133,
    downvotes: 15,
    rating: 4.6,
  },
  {
    id: 7,
    title: "Whispers of the Ocean",
    author: "Liam Johnson",
    readTime: "4 min read",
    imageUrl: "https://images.pexels.com/photos/3146245/pexels-photo-3146245.jpeg",
    description: "Mystical stories inspired by the sea.",
    href: "/story/7",
    categories: ["Adventure", "Fantasy"],
    upvotes: 76,
    downvotes: 4,
    rating: 4.0,
  },
  {
    id: 8,
    title: "Midnight in the City",
    author: "Ella Morgan",
    readTime: "5 min read",
    imageUrl: "https://images.pexels.com/photos/8261180/pexels-photo-8261180.jpeg",
    description: "Discover the hidden beauty of cities after dark.",
    href: "/story/8",
    categories: ["Thriller", "Drama"],
    upvotes: 149,
    downvotes: 11,
    rating: 4.9,
  },
  {
    id: 9,
    title: "Echoes of the Mountains",
    author: "Daniel Carter",
    readTime: "8 min read",
    imageUrl: "https://images.pexels.com/photos/6497611/pexels-photo-6497611.jpeg",
    description: "An adventure through the peaks and valleys of life.",
    href: "/story/9",
    categories: ["Adventure", "Drama"],
    series: {
      ...getSeries("eternal"),
      chapter: 4,
      chapterName: "Echoes of the Mountains",
    },
    upvotes: 120,
    downvotes: 9,
    rating: 4.4,
  },
  {
    id: 10,
    title: "Dancing with Shadows",
    author: "Isabella Reed",
    readTime: "6 min read",
    imageUrl: "https://images.pexels.com/photos/4759930/pexels-photo-4759930.jpeg",
    description: "Exploring the mysteries hidden in twilight hours.",
    href: "/story/10",
    categories: ["Thriller", "Fantasy"],
    upvotes: 101,
    downvotes: 7,
    rating: 4.2,
  },
  {
    id: 11,
    title: "The Bookstore Café",
    author: "Oliver Hayes",
    readTime: "5 min read",
    imageUrl: "https://images.pexels.com/photos/158018/surf-woman-mar-surfer-158018.jpeg",
    description: "Where stories are brewed with every cup of coffee.",
    href: "/story/11",
    categories: ["Comedy", "Romance"],
    upvotes: 89,
    downvotes: 5,
    rating: 4.0,
  },
  {
    id: 12,
    title: "Starlit Serenade",
    author: "Maya Thompson",
    readTime: "7 min read",
    imageUrl: "https://images.pexels.com/photos/1001445/pexels-photo-1001445.jpeg",
    description: "A heartfelt journey of two souls finding harmony under a sky full of stars.",
    href: "/story/12",
    categories: ["Romance", "Fantasy"],
    upvotes: 140,
    downvotes: 10,
    rating: 4.9,
  },

  // Extra Desire series chapters
  {
    id: 13,
    title: "Velvet Whispers: Hidden Promises",
    author: "Jane Doe",
    readTime: "5 min read",
    description: "The Parisian night fades, but hidden promises linger in the heart.",
    imageUrl: "https://images.pexels.com/photos/1535162/pexels-photo-1535162.jpeg",
    href: "/story/13",
    categories: ["Romance", "Drama"],
    series: {
      ...getSeries("desire"),
      chapter: 2,
      chapterName: "Hidden Promises",
    },
    upvotes: 102,
    downvotes: 6,
    rating: 4.5,
  },
  {
    id: 14,
    title: "Velvet Whispers: Flames of Longing",
    author: "Jane Doe",
    readTime: "7 min read",
    description: "Desire takes center stage as longing turns into an unspoken fire.",
    imageUrl: "https://images.pexels.com/photos/1142950/pexels-photo-1142950.jpeg",
    href: "/story/14",
    categories: ["Romance", "Drama"],
    series: {
      ...getSeries("desire"),
      chapter: 3,
      chapterName: "Flames of Longing",
    },
    upvotes: 118,
    downvotes: 9,
    rating: 4.6,
  },

  // Eternal series chapters
  {
    id: 15,
    title: "Eternal Bonds: The Awakening",
    author: "Daniel Carter",
    readTime: "6 min read",
    imageUrl: "https://images.pexels.com/photos/381739/pexels-photo-381739.jpeg",
    description: "The journey begins with a mysterious awakening under starlit skies.",
    href: "/story/15",
    categories: ["Adventure", "Fantasy"],
    series: {
      ...getSeries("eternal"),
      chapter: 1,
      chapterName: "The Awakening",
    },
    upvotes: 132,
    downvotes: 7,
    rating: 4.7,
  },
  {
    id: 16,
    title: "Eternal Bonds: Crossing Shadows",
    author: "Daniel Carter",
    readTime: "7 min read",
    imageUrl: "https://images.pexels.com/photos/210186/pexels-photo-210186.jpeg",
    description: "Friendship is tested as shadows rise along their journey.",
    href: "/story/16",
    categories: ["Adventure", "Drama"],
    series: {
      ...getSeries("eternal"),
      chapter: 2,
      chapterName: "Crossing Shadows",
    },
    upvotes: 121,
    downvotes: 6,
    rating: 4.5,
  },
  {
    id: 17,
    title: "Eternal Bonds: Flame and Frost",
    author: "Daniel Carter",
    readTime: "7 min read",
    imageUrl: "https://images.pexels.com/photos/355423/pexels-photo-355423.jpeg",
    description: "Opposites clash when the travelers face a trial of fire and ice.",
    href: "/story/17",
    categories: ["Fantasy", "Drama"],
    series: {
      ...getSeries("eternal"),
      chapter: 3,
      chapterName: "Flame and Frost",
    },
    upvotes: 127,
    downvotes: 10,
    rating: 4.6,
  },
];

const netVotes = (blog) => blog.upvotes - blog.downvotes;

const calculateScore = (blog) => {
  let score = netVotes(blog);
  score += blog.series ? 5 : 0;
  score += blog.rating * 10;
  return score;
};

export const getEditorsPick = () => {
  return blogs.reduce((best, blog) =>
    calculateScore(blog) > calculateScore(best) ? blog : best
  );
};

export const getFeaturedBlogs = (count = 3) => {
  return [...blogs]
    .sort((a, b) => calculateScore(b) - calculateScore(a))
    .slice(0, count);
};

export const getFeaturedBlogsByCategory = (category, count = 3) => {
  return blogs
    .filter((blog) => blog.categories.includes(category))
    .sort((a, b) => calculateScore(b) - calculateScore(a))
    .slice(0, count);
};

export const getBlogsByCategory = (category) => {
  return blogs.filter((blog) =>
    blog.categories.includes(category)
  );
};

export const getFeaturedStories = (count = 3) => {
  return [...blogs]
    .filter((blog) => !blog.series) // only series blogs
    .sort((a, b) => calculateScore(b) - calculateScore(a))
    .slice(0, count);
};

export const getBlogStories = () => {
  return blogs
    .filter((blog) => !blog.series);
};

export const getFeaturedSeries = (count = 3) => {
  return series
    .sort((a, b) => calculateScore(b) - calculateScore(a))
    .slice(0, count);
};

export const getBlogSeriesByName = (name) => {
  return blogs
    .filter((blog) => blog.series?.title === name)
    .sort((a, b) => a.series.chapter - b.series.chapter); // order by chapter
};

export const getBlogSeries = () => {
  return series;
};

export const getBlogId = (id, chapter) => {
  return blogs.find(blog => blog.series?.id === id && blog.series?.chapter === Number(chapter))?.id;
}
